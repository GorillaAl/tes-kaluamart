...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user = Account.object.create_user(first_name=first_name,
                                              last_name=last_name,
                                              email=email,
                                              username=username,
                                              password=password)
            ...
            # messages.success(
            #     request,
            #     'Pendaftaran Berhasil, kami akan mengirim verifikasi ke email anda. Silakan verifikasi'
            # )
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            try:
                ...
                # Get the cart item from the user to  access his prodcut varitians
            ...
            messages.error(request, 'Gagal Login, Mungkin kamu lelah')
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'Selamat')
        return redirect('login')
    else:
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            user_form.save()
            profile_form.save()
            messages.success(request, 'Profile kamu sudah di perbarui')
...
def change_password(request):
    if request.method == 'POST':
        ...
        if password_baru == confirm_password:
            success = user.check_password(password_lama)
            if success:
                ...
                messages.error(request,
                               'Silahkan masukan pasword lama dengan benar')
                return redirect('change_password')
        else:
